%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\documentclass[xcolor=x11names,compress, draft]{beamer}
...
\begin{frame}[fragile]
  \frametitle{Anisotropy: a computational challenge}
	...
	\item Many important nuclear applications have strong anisotropies
	 ...
	 \item Active interrogation of cargo
	 \end{itemize}
	\item These are difficult to capture with current tools:
	 \begin{itemize}
	 \item Ray affects with deterministic methods
	 \item Too slow with analog MC
	 \item Insufficient acceleration of MC with hybrid methods
	 \end{itemize}
	\end{itemize}
  	\end{column}
 	%
